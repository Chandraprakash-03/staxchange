...
export default function ConvertPage() {
	...
	// Check if project has target tech stack - but only redirect once
	useEffect(() => {
		if (currentProject && !currentProject.targetTechStack && !redirected) {
			console.log("No target tech stack found, redirecting to selection");
			setRedirected(true);
			navigateToStep("select");
		}
	}, [currentProject, navigateToStep, redirected]);
	...
	const handleCancelConversion = () => {
		...
		setConversionStarted(false);
		// Navigate back to selection after cancel
		navigateToStep("select");
	};
	...
	// Show loading only when explicitly loading, not when project is missing
	...
	// Show error message if no project and not redirected
	if (!currentProject && !redirected) {
		return (
			<div className="min-h-screen bg-gray-50 flex items-center justify-center">
				<div className="text-center">
				...
				<div className="max-w-6xl mx-auto">
					...
					{!conversionStarted ? (
						...
						<ConversionProgressMonitor
							...
							conversionJob={activeConversion!}
